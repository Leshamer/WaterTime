...
cups.onclick = function(e){
    var target = e.target;
    if((target.classList.contains('check-icon')) && (!(target.classList.contains('active')))){
...
function updateCurrentDate() {
    ...
    const formattedDate = `${now.getDate() < 10 ? '0' : ''}${now.getDate()}.${(now.getMonth() + 1) < 10 ? '0' : ''}${now.getMonth() + 1}.${now.getFullYear()}`;
